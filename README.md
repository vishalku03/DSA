# Data Structures & Algorithms (DSA)

This repository contains **topic-wise implementations of Data Structures and Algorithms in C++**.
It is designed to help anyone **understand core DSA concepts clearly** and prepare effectively for **coding interviews**.

All topics are organized in **separate folders** for easy navigation, learning, and revision.

---

## Folder Structure

- **Array** – Basic to advanced array problems
- **String** – Common string manipulation techniques
- **Linked_list** – Singly and doubly linked list operations
- **Binary_search** – Binary search and related problems
- **Recursion** – Recursive problem-solving patterns
- **Stack_-_Queue** – Stack and queue implementations and applications

Each folder contains **clean, well-written C++ code** focused on logic and clarity.

---

## Purpose of This Repository

- Build strong **DSA fundamentals**
- Practice **problem-solving skills**
- Prepare for **technical interviews**
- Quick **revision before exams or interviews**

---

##  Work Ownership

> **All the code in this repository is written and maintained by me.**  
> Each implementation is created from scratch with a focus on clarity, correctness, and understanding of core concepts.

---

## 🛠 Language Used
- **C++**


   g++ filename.cpp
   ./a.out
